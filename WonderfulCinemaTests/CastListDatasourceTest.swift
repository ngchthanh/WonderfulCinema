...
class CastListDatasourceTest: XCTestCase {
    ...
    func testAPI() throws {
        let datasource = CastListDatasource(movieID: 123, movieAPI: MovieAPI())
        let exp = expectation(description: "🍏🍏🍏 ")
        let errorExp = expectation(description: "Error")
        datasource.$casts.dropFirst().sink(receiveValue: { casts in
            XCTAssertFalse(casts.isEmpty, "movies is empty ⚠️⚠️⚠️⚠️⚠️")
            if !casts.isEmpty {
                exp.fulfill()
            }
        }).store(in: &bag)
